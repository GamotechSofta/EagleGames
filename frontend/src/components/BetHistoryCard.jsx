...
const statusTone = (label) => {
  ...
  return 'text-gray-200 border-[#374151] bg-[#1f2937]';
};
...
const BetHistoryCard = ({
  ...
  return (
    <div
      className={`rounded-xl border-2 p-3 sm:p-3.5 text-white transition-[box-shadow,border-color,background-color] duration-200 relative ${
        isWin
          ...
          : isLost
            ...
            : 'bg-[#111827] border-[#374151] shadow-sm hover:border-[#1a74e5]/25'
      }`}
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <span className="text-[11px] sm:text-xs text-gray-400 font-medium">#{index}</span>
        <span
      ...
      <div className="flex items-center justify-between gap-2 text-[11px] sm:text-xs mb-2">
        ...
        <div className="flex items-center gap-1.5 min-w-0 justify-end">
          <span className="font-mono text-white truncate" title={betId}>
            {sid}
          </span>
          <button
            type="button"
            onClick={copyId}
            className="shrink-0 p-1 rounded text-gray-400 hover:text-[#1a74e5] hover:bg-[#374151] transition-colors"
      ...
      <div className="space-y-1 text-[11px] sm:text-xs">
        <div className="flex justify-between gap-3">
          ...
          <span className="text-white text-right min-w-0 font-medium">{gameLabel}</span>
        </div>
        <div className="flex justify-between gap-3 items-center">
          <span className="text-gray-400 shrink-0">Bet</span>
          <div className="text-white text-right min-w-0 font-semibold tabular-nums">
            {betValue}
          </div>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-gray-400 shrink-0">Bet amount</span>
          <span className="text-white text-right font-semibold tabular-nums">{fmtRupee(stake)}</span>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-gray-400 shrink-0">Win amount</span>
          <span
        ...
        <div className="flex justify-between gap-3">
          ...
          <span className="text-gray-300 text-right leading-snug">{timeFormatted}</span>
        </div>
      </div>
