    ...
    <div className="min-h-screen bg-[#374151] text-white px-3 sm:px-4 pt-3 pb-28">
      <div className="w-full max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-lg sm:text-xl font-extrabold tracking-wide truncate text-[#1a74e5]">MARKET RESULT HISTORY</h1>
        </div>

        {/* Date row */}
        <div className="rounded-2xl bg-[#111827] border-2 border-[#374151] p-3 mb-4 shadow-sm">
          <ResultDatePicker
            value={selectedDate}
            ...
            buttonClassName="px-4 py-2 rounded-full bg-[#1f2937] border-2 border-[#374151] text-white font-bold text-sm shadow-sm hover:border-[#1a74e5] focus:border-[#1a74e5] transition-colors"
          />
        </div>
        ...
        <div className="space-y-3">
          {rows.length === 0 ? (
            <div className="rounded-2xl border-2 border-[#374151] bg-[#111827] p-6 text-center text-gray-500">
              No markets found.
            </div>
          ) : (
            rows.map((r) => (
              <div
                key={r.id}
                className="rounded-2xl bg-[#111827] border-2 border-[#374151] px-5 py-4 shadow-sm flex items-center justify-between gap-4"
              >
                <div className="font-extrabold tracking-wide text-white truncate">{r.name.toUpperCase()}</div>
                <div className="font-extrabold tracking-wide text-[#1a74e5] shrink-0">{r.result}</div>
              </div>
            ))
          )}
        </div>
