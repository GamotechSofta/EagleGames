...
const TriplePanaBulkBid = ({ market, title }) => {
    ...
    const handleAddBid = () => {
        ...
        if (!n) {
            showWarning(bid.pleaseEnterTriplePana);
            return;
        }
    ...
    const bidsList = (
        <>
            ...
            <div className="space-y-2">
                {bids.map((bid) => (
                    <div key={bid.id} className="grid grid-cols-4 gap-1 sm:gap-2 text-center items-center py-2.5 px-2 bg-[#1f2937] rounded-lg border-2 border-[#374151] text-sm">
                        ...
                        <div>
                            <div className="flex flex-col gap-3 mt-2 mb-4 px-1">
                                <div className="flex flex-row items-center gap-2">
                                    <label className="text-gray-200 text-sm font-medium shrink-0 w-28">{bid.enterPana}:</label>
                                    <input
                                        type="text"
                                        inputMode="numeric"
                                        value={inputNumber}
                                        ...
                                        placeholder="Pana"
                                        maxLength={3}
                                        className={`flex-1 min-w-0 bg-[#111827] border-2 border-[#374151] text-white placeholder-gray-400 rounded-xl py-2.5 min-h-[40px] px-4 text-left text-sm focus:ring-2 focus:outline-none ${isPanaInvalid ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'focus:ring-[#1B3150] focus:border-[#1a74e5]'}`}
                                    />
                                </div>
                                <div className="flex flex-row items-center gap-2">
                                ...
                                <div className="flex flex-row items-center gap-2">
                                    ...
                                    <div className="flex-1 min-w-0 grid grid-cols-5 gap-2">
                                        {quickPointValues.map((pts) => (
                                            <button key={pts} type="button" onClick={() => handleQuickPointClick(pts)} className="py-2 min-h-[36px] rounded-lg border-2 border-[#374151] bg-[#111827] text-sm font-medium text-white hover:border-[#1a74e5] active:scale-95">
                                                {pts}
                                            </button>
                                        ))}
                                    </div>
