        ...
        return (
            <AdminLayout onLogout={handleLogout} title="Bookie Detail">
                <div className="flex flex-col items-center justify-center min-h-[40vh]">
                    ...
                    <Link to="/revenue" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500 text-gray-800 font-semibold">
    ...
    return (
        <AdminLayout onLogout={handleLogout} title="Bookie Detail">
            <div className="min-w-0 max-w-full space-y-4 sm:space-y-6">
                {/* Back + Header */}
                <div>
                    <Link to="/revenue" className="text-gray-400 hover:text-blue-500 text-sm inline-flex items-center gap-1 mb-2">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${bookie?.status === 'active' ? 'bg-blue-500/20 text-emerald-600' : 'bg-red-500/20 text-red-500'}`}>
                            {bookie?.status === 'active' ? 'Active' : 'Inactive'}
                        </span>
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-500/15 text-blue-500">
                        ...
                        {PRESETS.map((p) => (
                            <button
                                ...
                                className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
                                    activePreset === p.id ? 'bg-blue-500 text-gray-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                }`}
                            >
                                {p.label}
                            </button>
                ...
                {/* Tabs */}
                <div className="flex flex-wrap gap-1.5 sm:gap-2 border-b border-gray-200 pb-2">
                    {TABS.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-semibold text-xs sm:text-sm transition-colors ${
                                activeTab === tab.id ? 'bg-blue-500 text-gray-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                            }`}
                        >
                            {tab.label}
                        </button>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200/60 relative overflow-hidden">
                                ...
                                <p className="text-sm sm:text-lg lg:text-xl font-bold text-gray-800 mt-1 truncate">{formatCurrency(revenue.totalBetAmount)}</p>
                            ...
                            <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200/60 relative overflow-hidden">
                                ...
                                <p className="text-sm sm:text-lg lg:text-xl font-bold text-red-500 mt-1 truncate">{formatCurrency(revenue.totalPayouts)}</p>
                            ...
                            <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200/60 relative overflow-hidden">
                                ...
                                <p className="text-sm sm:text-lg lg:text-xl font-bold text-blue-400 mt-1 truncate">{formatCurrency(revenue.bookieShare)}</p>
                            </div>
                            <div className={`bg-white rounded-xl p-3 sm:p-4 border relative overflow-hidden ${revenue.adminProfit >= 0 ? 'border-emerald-500/40' : 'border-red-500/40'}`}>
                                <div className={`absolute top-0 left-0 right-0 h-1 ${revenue.adminProfit >= 0 ? 'bg-blue-500' : 'bg-red-500'}`} />
                        ...
                        <div className="bg-white rounded-xl border border-gray-200/60 p-3 sm:p-5">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between py-1.5 border-b border-gray-200/40">
                                    ...
                                    <span className="text-gray-800 font-medium">{formatCurrency(revenue.totalBetAmount)}</span>
                                </div>
                                <div className="flex justify-between py-1.5 border-b border-gray-200/40">
                                ...
                                <div className="flex justify-between py-1.5 border-b border-gray-200/40">
                                    ...
                                    <span className="text-blue-400 font-medium">- {formatCurrency(revenue.bookieShare)}</span>
                                </div>
                                <div className="flex justify-between py-1.5 border-b border-gray-200/40">
                                ...
                                <div className="flex justify-between py-2 font-bold">
                                    <span className="text-blue-500">Admin Profit</span>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-white rounded-lg p-3 text-center border border-gray-200/40">
                                <p className="text-[10px] sm:text-xs text-gray-500 uppercase">Total Users</p>
                                <p className="text-lg sm:text-xl font-bold text-gray-800">{formatNumber(data?.totalUsers)}</p>
                            </div>
                            <div className="bg-white rounded-lg p-3 text-center border border-gray-200/40">
                                <p className="text-[10px] sm:text-xs text-gray-500 uppercase">Win Rate</p>
                                <p className="text-lg sm:text-xl font-bold text-gray-800">{revenue.winRate}%</p>
                            </div>
                            <div className="bg-white rounded-lg p-3 text-center border border-gray-200/40">
                                <p className="text-[10px] sm:text-xs text-gray-500 uppercase">Total Bets</p>
                                <p className="text-lg sm:text-xl font-bold text-gray-800">{formatNumber(revenue.totalBetCount)}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'users' && (
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-3 sm:px-5 py-3 border-b border-gray-200">
                            <h3 className="text-sm sm:text-base font-semibold text-gray-800">Users ({formatNumber(users.length)})</h3>
                        </div>
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-700/40">
                                    {users.map((u) => (
                                        <tr key={u.userId} className="hover:bg-gray-100/20 transition-colors">
                                            <td className="px-4 py-2.5">
                                                <div className="flex items-center gap-2">
                                                    <div className={`w-2 h-2 rounded-full shrink-0 ${u.isActive ? 'bg-emerald-400' : 'bg-red-400'}`} />
                                                    <Link to={`/all-users/${u.userId}`} className="font-medium text-gray-800 hover:text-blue-500 transition-colors">{u.username}</Link>
                                                </div>
                                                {u.phone && <p className="text-[11px] text-gray-500 ml-4">{u.phone}</p>}
                                            </td>
                                            <td className="text-right px-3 py-2.5 text-gray-600">{formatNumber(u.totalBets)}</td>
                                            <td className="text-right px-3 py-2.5 text-gray-800 font-medium">{formatCurrency(u.totalAmount)}</td>
                                            <td className="text-right px-3 py-2.5 text-red-500">{formatCurrency(u.totalPayout)}</td>
                                            <td className="text-right px-3 py-2.5 text-xs">
                                                <span className="text-emerald-600">{u.wins}</span>
                                                <span className="text-gray-600 mx-0.5">/</span>
                                                <span className="text-red-500">{u.losses}</span>
                                            </td>
                                            <td className={`text-right px-4 py-2.5 font-semibold ${u.profit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                                ...
                                <div key={u.userId} className="p-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <div className={`w-2 h-2 rounded-full shrink-0 ${u.isActive ? 'bg-emerald-400' : 'bg-red-400'}`} />
                                            <Link to={`/all-users/${u.userId}`} className="font-medium text-gray-800 text-sm hover:text-blue-500 truncate">{u.username}</Link>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 text-[11px]">
                                        <div>
                                            ...
                                            <span className="text-gray-800">{formatCurrency(u.totalAmount)}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Pay: </span>
                                            <span className="text-red-500">{formatCurrency(u.totalPayout)}</span>
                                        </div>
                                        <div>
                ...
                {activeTab === 'bets' && (
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-3 sm:px-5 py-3 border-b border-gray-200">
                            <h3 className="text-sm sm:text-base font-semibold text-gray-800">Recent Bets ({formatNumber(bets.length)})</h3>
                        </div>
                            ...
                            {bets.map((b) => (
                                <div key={b._id} className="p-3 sm:p-4 hover:bg-gray-100/20 transition-colors">
                                    <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <span className="text-blue-500 font-mono font-medium text-sm">{b.betNumber}</span>
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
                                            b.status === 'won' ? 'bg-emerald-900/50 text-emerald-600' :
                                            b.status === 'lost' ? 'bg-red-50 text-red-500' :
                                            'bg-gray-200/50 text-gray-600'
                                        }`}>{b.status}</span>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                                        <span><span className="text-gray-500">Player:</span> <span className="text-gray-800">{b.username}</span></span>
                                        <span><span className="text-gray-500">Market:</span> <span className="text-gray-600">{b.marketName}</span></span>
                                        <span><span className="text-gray-500">Amt:</span> <span className="text-gray-800 font-mono">{formatCurrency(b.amount)}</span></span>
                                        {b.payout > 0 && (
                                            <span><span className="text-gray-500">Pay:</span> <span className="text-emerald-600 font-mono">{formatCurrency(b.payout)}</span></span>
                                        )}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium truncate">{bookie.username}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-blue-500/15 flex items-center justify-center shrink-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium">{bookie.phone || '—'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium truncate">{bookie.email || '—'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-blue-500/15 flex items-center justify-center shrink-0">
                                    <FaPercent className="w-4 h-4 text-blue-400" />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] text-gray-500 uppercase tracking-wider">Commission</p>
                                    <p className="text-blue-400 text-sm font-bold">{bookie.commissionPercentage}%</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-blue-500/15 flex items-center justify-center shrink-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium">{formatNumber(data?.totalUsers)}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-gray-800 text-sm font-medium">{bookie.createdAt ? new Date(bookie.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${bookie.status === 'active' ? 'bg-blue-500/15' : 'bg-red-500/15'}`}>
                                    <div className={`w-3 h-3 rounded-full ${bookie.status === 'active' ? 'bg-emerald-400' : 'bg-red-400'}`} />
                                </div>
                                <div className="min-w-0">
                                    <p className="text-[10px] text-gray-500 uppercase tracking-wider">Status</p>
                                    <p className={`text-sm font-medium capitalize ${bookie.status === 'active' ? 'text-emerald-600' : 'text-red-500'}`}>{bookie.status}</p>
                                </div>
                            </div>
                        </div>
                    </div>
